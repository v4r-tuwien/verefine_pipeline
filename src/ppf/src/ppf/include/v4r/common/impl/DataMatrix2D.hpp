...
class DataMatrix2D : public DataContainer {
 public:
  ...
  virtual inline void setZero() {
    std::cout << "[DataMatrix2D::setZero] Not available!" << std::endl;
  }
 ...
 */
template <class T>
DataMatrix2D<T>::DataMatrix2D(const DataContainer::Type &_type) : DataContainer(_type), rows(0), cols(0) {}
...
void DataMatrix2D<T>::read(const std::string &filename, DataMatrix2D<T> &mat) {
  ...
  if (in.is_open()) {
    ...
    throw std::runtime_error("[DataMatrix2D::read] File not found!");
}
