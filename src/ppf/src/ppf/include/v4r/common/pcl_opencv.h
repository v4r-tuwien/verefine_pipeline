...
class PCLOpenCVConverter {
 private:
  ...
  Intrinsics::ConstPtr cam_;  ///< RGB camera parameters (used for re-projection if point cloud is not organized)
  ...
  cv::Mat fillMatrix(Converter converter);

 public:
  explicit PCLOpenCVConverter(const typename pcl::PointCloud<PointT>::ConstPtr cloud = nullptr)
