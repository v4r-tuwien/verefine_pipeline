...
class  ZBuffering {
 private:
  ...
  Intrinsics::ConstPtr cam_;   ///< camera parameters
  ...
  /**
   * @brief return surface normal belonging to a certain point index in the cloud (used to minimize code redundancy due
   * to template specialization on PCL point type)
   * @param cloud input point cloud
   * @param index point index
   * @return surface normal for given point index of the point cloud
   */
  Eigen::Vector4f getNormal(const pcl::PointCloud<PointT> &cloud, size_t index) const;

  /**
   * @brief validation function to check whether surface normals are available
   * @param cloud point cloud
   * @return true if surface normals are available and valid
   */
  inline bool hasValidNormals(const pcl::PointCloud<PointT> &cloud) const {
    return pcl::traits::has_normal<PointT>::value || (cloud_normals_ && cloud_normals_->size() == cloud.size());
  }

 public:
  ZBuffering(const Intrinsics::ConstPtr &cam, const ZBufferingParameter &p = ZBufferingParameter())
  : param_(p), cam_(cam) {}
   ...
   */
  typename pcl::PointCloud<PointT>::Ptr renderPointCloud(const pcl::PointCloud<PointT> &cloud, size_t subsample = 1);
