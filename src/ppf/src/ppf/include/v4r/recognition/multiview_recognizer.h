...
#include <v4r/recognition/local_object_model.h>
...
struct MultiviewRecognizerParameter {
...
class MultiviewRecognizer : public RecognitionPipeline<PointT> {
 private:
  ...
  using RecognitionPipeline<PointT>::transform_to_world_;
  using RecognitionPipeline<PointT>::transform_to_world_set_;
  using PointTWithNormal = typename RecognitionPipeline<PointT>::PointTWithNormal;
  ...
  struct View {
    ...
    std::map<std::string, pcl::CorrespondencesPtr> local_obj_hypotheses_;  ///< stores feature correspondences
