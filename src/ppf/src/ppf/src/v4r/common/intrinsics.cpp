...
Intrinsics Intrinsics::load(const std::string& filename) {
  ...
  if (boost::algorithm::ends_with(filename, ".yaml") || boost::algorithm::ends_with(filename, ".yml") ||
    ...
    const std::vector<std::string> cam_matrix = {"cameraMatrix", "camera_matrix", "matrix"};
    const auto it = std::find_if(cam_matrix.begin(), cam_matrix.end(),
                                 [&fs](const auto& name) -> bool { return !fs[name].empty(); });
    if (it != cam_matrix.end())
      fs[*it] >> m;
    ...
    if (!fs["resolution"].empty()) {
      ...
      (*node++)["width"] >> width;
      (*node)["height"] >> height;
      LOG(INFO) << "got camera resolution " << width << "x" << height;
    }
    if (width == 0 || height == 0)
