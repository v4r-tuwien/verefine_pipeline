...
namespace v4r {
bool isInlier(const Eigen::Vector3f &point, const Eigen::Vector4f &normal, const Eigen::Vector4f &plane,
...
bool isInlier(const Eigen::Vector3f &point, const Eigen::Vector4f &normal, const Eigen::Vector4f &plane,
  ...
  if (fabs(dist2plane(point, plane)) < distThreshold) {
    if (doNormalTest) {
...
bool isInPlane(const Eigen::Vector4f &plane1, const Eigen::Vector3f &centerPlane2, float distThreshold) {
  return fabs(dist2plane(centerPlane2, plane1)) < distThreshold;
}
    ...
    for (size_t j = 0; j < colsOfPatches; j++) {
      ...
      if (planeId) {
        ...
        if (planes[i][j].nrInliers > minAbsBlockInlier) {
          for (size_t k = 0; k < param_.patchDim_; k++) {
            for (size_t l = 0; l < param_.patchDim_; l++) {
              // mark the points in debug:
              // honestly we should still check if the point is an inlier
              const Eigen::Vector4f &normal =
                  normal_cloud_->at(j * param_.patchDim_ + l, i * param_.patchDim_ + k)
                      .getNormalVector4fMap();  // normals.at<Eigen::Vector4f>(i*param_.patchDim_+k,
                                                // j*param_.patchDim_+l);
              const Eigen::Vector3f &point =
                  cloud_->at(j * param_.patchDim_ + l + 1, i * param_.patchDim_ + k + 1)
                      .getVector3fMap();  // points.at<Eigen::Vector4f>(i*param_.patchDim_+k+1, j*param_.patchDim_+l+1)
...
template <typename PointT>
cv::Mat PlaneExtractorTile<PointT>::generateColorCodedTexture() const {
...
template <typename PointT>
cv::Mat PlaneExtractorTile<PointT>::generateColorCodedTextureDebug() const {
  cv::Mat colorMap(1, 64 * 48, CV_8UC3);  /// TODO Is this is a bug (why fixed???)
  colorMap.at<cv::Vec3b>(0) = cv::Vec3b(0, 0, 0);
  colorMap.at<cv::Vec3b>(1) = cv::Vec3b(0, 0, 200);
  colorMap.at<cv::Vec3b>(2) = cv::Vec3b(0, 200, 0);
...
template <typename PointT>
Eigen::Vector4f PlaneExtractorTile<PointT>::calcPlaneFromMatrix(
    const PlaneExtractorTile<PointT>::PlaneMatrix &m) const {
  Eigen::Matrix3d mat;
  mat << m.xx, m.xy, m.xz, m.xy, m.yy, m.yz, m.xz, m.yz, m.zz;
  ...
  if (Eigen::Vector3f::UnitZ().dot(plane4.head(3)) > 0.f)
    plane4 = -plane4;
...
template <typename PointT>
void PlaneExtractorTile<PointT>::replace(int from, int to, int maxIndex) {
...
template <typename PointT>
int PlaneExtractorTile<PointT>::allocateMemory() {
  minAbsBlockInlier = param_.patchDim_ * param_.patchDim_ * param_.minBlockInlierRatio_;
  CHECK(cloud_->width % param_.patchDim_ == 0 && cloud_->height % param_.patchDim_ == 0);
  colsOfPatches = cloud_->width / param_.patchDim_;
  rowsOfPatches = cloud_->height / param_.patchDim_;

  size_t nrMatrices = colsOfPatches * rowsOfPatches;
  matrices.resize(nrMatrices);
  planeMatrices.resize(nrMatrices);

  segmentation = cv::Mat(cloud_->height, cloud_->width, CV_32SC1);
  segmentation.setTo(0);
  ...
  //    if(normals.cols!=cloud_->width || normals.rows!=cloud_->height)
  {
    ...
    centerPoints =
        std::vector<std::vector<Eigen::Vector3f>>(rowsOfPatches, std::vector<Eigen::Vector3f>(colsOfPatches));
...
template <typename PointT>
void PlaneExtractorTile<PointT>::calculatePlaneSegments(bool doNormalTest) {
    ...
    for (size_t j = 0; j < colsOfPatches; j++) {
      ...
      for (size_t m = 0; m < param_.patchDim_; m++) {
        for (size_t n = 0; n < param_.patchDim_; n++) {
          const PointT &p = cloud_->at(
              j * param_.patchDim_ + n,
              i * param_.patchDim_ + m);  // points.at<Eigen::Vector4f>(i*param_.patchDim_+m+1,j*param_.patchDim_+n+1);
          if (pcl::isFinite(p)) {
            pm.addPoint(p.getVector3fMap());
          }
        }
      }
      matrices[i * colsOfPatches + j] = pm;
    }
  }
  ...
  for (size_t i = 0; i < rowsOfPatches; i++) {
    for (size_t j = 0; j < colsOfPatches; j++) {
      ...
      if (m.nrPoints > minAbsBlockInlier) {
        ...
        for (size_t k = 0; k < param_.patchDim_; k++) {
          for (size_t l = 0; l < param_.patchDim_; l++) {
            size_t u = l + j * param_.patchDim_;
            size_t v = k + i * param_.patchDim_;
            ...
            // TODO: remove this isInlier.... or at least store the norm for this so it does not have to be recalculated
            // for every pixel
            if (isInlier(p, N, plane, cosThreshold, distThreshold, doNormalTest)) {  // distance < inlierDistance
        ...
        if (planes[i][j].nrInliers > minAbsBlockInlier) {
          for (size_t k = 0; k < param_.patchDim_; k++) {
            for (size_t l = 0; l < param_.patchDim_; l++) {
              ...
              // TODO: remove this isInlier.... or at least store the norm for this so it does not have to be
...
void PlaneExtractorTile<PointT>::rawPatchClustering() {
  ...
  for (size_t i = 0; i < rowsOfPatches; i++) {
    ...
    for (size_t j = 0; j < colsOfPatches; j++) {
      ...
      const Eigen::Vector4f &currentPatch =
          currentPlaneSeg.plane;  //( currentPlaneSeg.x, currentPlaneSeg.y, currentPlaneSeg.z, currentPlaneSeg.d);
      const Eigen::Vector3f &currentCenter = centerPoints[i][j];

      // test if plane is valid
      if (currentPlaneSeg.nrInliers > minAbsBlockInlier) {
        bool gotSet = false;
        float distThreshold = param_.maxInlierBlockDist_;
        float cosThreshold = minCosBlockAngle;
        if (param_.useVariableThresholds_) {
          // read it from the buffer
          const Eigen::Vector4f &thresholds = thresholdsBuffer[i][j];
          distThreshold = thresholds[0];
        ...
        if (i > 0) {
          ...
          if (j > 0 && !alreadyTested) {
            // do upper left
            int newId = patchIds.at<int>(
                i - 1, j - 1);  // it is only testing for blocks from the past(so the threshold is already checked
            if (newId) {
          ...
          if (!alreadyTested || j == 0) {  // forget alreadyTested. it does not make sense
            ...
            if (newId) {
              ...
              const Eigen::Vector4f &newPatch =
                  testPlaneSeg.plane;  //(currentPlaneSeg.x,currentPlaneSeg.y,currentPlaneSeg.z,currentPlaneSeg.d);
          ...
          if (j + 1 < colsOfPatches) {
            ...
            if (newId) {
              ...
              const Eigen::Vector4f &newPatch =
                  testtPlaneSeg.plane;  //(currentPlaneSeg.x,currentPlaneSeg.y,currentPlaneSeg.z,currentPlaneSeg.d);
...
template <typename PointT>
void PlaneExtractorTile<PointT>::postProcessing1Direction(const int offsets[][2], bool doNormalTest, bool reverse,
                                                          bool zTest) {
      ...
      if (currentId > 0) {
        ...
        if (currentId != oldId || !oldPlaneTTL) {
          ...
          for (int k = 0; k < 9; k++) {
            ...
            if (_i >= 0 && _j >= 0 && _i < (int)rowsOfPatches && j < (int)colsOfPatches) {
          ...
          for (size_t k = 0; k < 4; k++) {
            ...
            if (_i >= 0 && _j >= 0 && _i < segmentation.rows && _j < segmentation.cols) {
              ...
              if (otherId <= 0 || zTest) {  // only do this if pixel is not yet set
                ...
                const Eigen::Vector3f &otherPoint =
                    cloud_->at(_j, _i).getVector3fMap();  // points.at<Eigen::Vector4f>(_i+1,_j+1);
                ...
                if (newDist < oldDist || !zTest) {
                  ...
                  if (doNormalTest)
                    otherNormal =
                        normal_cloud_->at(_j, _i).getNormalVector4fMap();  // normals.at<Eigen::Vector4f>(_i,_j);
...
template <typename PointT>
void PlaneExtractorTile<PointT>::postProcessing(bool doNormalTest, bool zTest) {
...
template <typename PointT>
PlaneExtractorTile<PointT>::PlaneExtractorTile(const PlaneExtractorParameter &p) : PlaneExtractor<PointT>(p) {
  ...
  minCosBlockAngleFunc = [](float z) -> float {
    ...
    float maxAngle_local =
        M_PI / 180.0f * std::min(alphamin + (z - zmin) * (z - zmin) / (zmax * zmax) * (alphamax - alphamin), 90.0f);
...
template <typename PointT>
void PlaneExtractorTile<PointT>::do_compute(const boost::optional<const Eigen::Vector3f> &search_axis) {
  (void)search_axis;
      ...
      if (newPlaneId) {
        ...
        if (planes[i][j].nrInliers > minAbsBlockInlier) {
          for (size_t k = 0; k < param_.patchDim_; k++) {
            for (size_t l = 0; l < param_.patchDim_; l++) {
              if (segmentation.at<int>(i * param_.patchDim_ + k, j * param_.patchDim_ + l) == -1) {
                ...
                const Eigen::Vector3f &point =
                    cloud_->at(j * param_.patchDim_ + l, i * param_.patchDim_ + k).getVector3fMap();
          ...
          for (size_t k = 0; k < param_.patchDim_; k++) {
            for (size_t l = 0; l < param_.patchDim_; l++) {
              if (segmentation.at<int>(i * param_.patchDim_ + k, j * param_.patchDim_ + l) == -1)
                segmentation.at<int>(i * param_.patchDim_ + k, j * param_.patchDim_ + l) =
                    0;  // segmentation is set to zero at every other points
            }
          }
        }
      }
...
#define PCL_INSTANTIATE_PlaneExtractorTile(T) template class  PlaneExtractorTile<T>;
PCL_INSTANTIATE(PlaneExtractorTile, V4R_PCL_XYZ_POINT_TYPES)
}  // namespace v4r
