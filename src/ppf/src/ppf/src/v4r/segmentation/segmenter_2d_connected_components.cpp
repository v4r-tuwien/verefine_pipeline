...
void ConnectedComponentsSegmenter<PointT>::segment() {
...

  {
    ...
    binary_cloud->points.resize(scene_->size());
    binary_cloud->is_dense = scene_->is_dense;

    for (size_t i = 0; i < scene_->size(); i++) {
      binary_cloud->points[i].getVector4fMap() = scene_->points[i].getVector4fMap();
      binary_cloud->points[i].intensity = 1.f;
    }
  }
    ...
    for (int v = 0; v < int(binary_cloud->height); v++) {
      if (binary_cloud->at(u, v).intensity != 0.f) {
        ...
        if ((u - 1) < 0 && (v - 1) < 0)  // top-left pixel
          ...
          {
            if (check((*binary_cloud)(u - 1, v), (*binary_cloud)(u, v))) {
              ...
              for (int kk = 2; kk < param_.wsize_ && !found; kk++) {
                ...
                if (!check((*binary_cloud)(u - kk, v), (*binary_cloud)(u, v)))
                  found = true;
              }
            ...
            if ((u - 1) == 0) {
              // check only top
              if (check((*binary_cloud)(u, v - 1), (*binary_cloud)(u, v))) {
                bool found = false;
                for (int kk = 2; kk < param_.wsize_ && !found; kk++) {
                  if ((v - kk) < 0)
                    continue;

                  if (!check((*binary_cloud)(u, v - kk), (*binary_cloud)(u, v)))
                    found = true;
                }
              ...
              if (left && top) {
                ...
                for (int dist = 2; dist < param_.wsize_ && !found; dist++) {
                  ...
                  if (!check((*binary_cloud)(u - dist, v), (*binary_cloud)(u, v)) &&
                      !check((*binary_cloud)(u, v - dist), (*binary_cloud)(u, v))) {
...
#define PCL_INSTANTIATE_ConnectedComponentsSegmenter(T) template class  ConnectedComponentsSegmenter<T>;
PCL_INSTANTIATE(ConnectedComponentsSegmenter, V4R_PCL_XYZ_POINT_TYPES)
}  // namespace v4r
