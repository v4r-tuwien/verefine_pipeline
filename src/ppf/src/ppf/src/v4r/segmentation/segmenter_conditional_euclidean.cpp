...
void ConditionalEuclideanSegmenter<PointT>::segment() {
  ...
  for (size_t idx = 0; idx < scene_->points.size(); idx++)  // iii = input indices iterator
  {
    ...
    while (cii < current_cluster.size()) {
      ...
      float cluster_tolerance = param_.distance_threshold_;
      if (param_.z_adaptive_) {
        float mult = std::max(scene_->points[current_cluster[cii]].z, 1.f);
        cluster_tolerance = param_.distance_threshold_ * mult;
      }
...
#define PCL_INSTANTIATE_ConditionalEuclideanSegmenter(T) template class  ConditionalEuclideanSegmenter<T>;
PCL_INSTANTIATE(ConditionalEuclideanSegmenter, V4R_PCL_XYZ_POINT_TYPES)
}  // namespace v4r
