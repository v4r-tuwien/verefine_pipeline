...
void OrganizedConnectedComponentSegmenter<PointT>::segment() {
  ...
  labels->resize(scene_->points.size());
  ...
  euclidean_cluster_comp->setDistanceThreshold(param_.distance_threshold_, param_.z_adaptive_);
...
#define PCL_INSTANTIATE_OrganizedConnectedComponentSegmenter(T) \
  template class  OrganizedConnectedComponentSegmenter<T>;
PCL_INSTANTIATE(OrganizedConnectedComponentSegmenter, V4R_PCL_XYZ_POINT_TYPES)
}  // namespace v4r
