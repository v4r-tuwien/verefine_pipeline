...
void SmoothEuclideanSegmenter<PointT>::segment() {
  CHECK(scene_ && normals_ && scene_->size() == normals_->size());
  clusters_.clear();
  ...
  float eps_angle_threshold_rad = pcl::deg2rad(param_.angular_threshold_deg_);
  ...
  for (size_t i = 0; i < scene_->size(); ++i) {
    ...
    while (sq_idx < seed_queue.size()) {
      ...
      float radius = param_.distance_threshold_;
      ...
      if (param_.z_adaptive_) {
        radius = param_.distance_threshold_ * (1 + (std::max(query_pt.z, 1.f) - 1.f));
      ...
      for (size_t j = 0; j < nn_indices.size(); j++) {
        ...
        const pcl::Normal &nn = normals_->points[nn_indices[j]];
    ...
    if (seed_queue.size() >= param_.min_cluster_size_ && seed_queue.size() <= param_.max_cluster_size_) {
      std::vector<int> r;
...
#define PCL_INSTANTIATE_SmoothEuclideanSegmenter(T) template class  SmoothEuclideanSegmenter<T>;
PCL_INSTANTIATE(SmoothEuclideanSegmenter, V4R_PCL_XYZ_POINT_TYPES)
}  // namespace v4r
