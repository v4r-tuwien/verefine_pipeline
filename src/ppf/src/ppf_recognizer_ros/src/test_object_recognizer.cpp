...
public:
    ...
    void callPPFRecognizerUsingCam(const sensor_msgs::PointCloud2::ConstPtr& msg)
    {
        std::cout << "Received point cloud.\n" << std::endl;

        if(!camera_info_set_)
        {
            ppf_recognizer_ros_msgs::set_camera set_cam_srv;
    ...
    bool checkKinect ()
    {
        ros::Subscriber sub_pc = n_->subscribe (topic_, 1, &PPFRecognizerDemo::checkCloudArrive, this);

        ros::Rate loop_rate (1);

        // construct associated camera_info topic from camera topic
        std::vector<std::string> split_result;
        boost::split(split_result, topic_, [](char c) { return c == '/'; });
        split_result.back() = "camera_info";
        std::string camera_info_topic = boost::join(split_result, "/");

        std::cout << "Checking if there is an associated camera info topic on " << camera_info_topic << "." << std::endl;

        ros::Subscriber cam_info_sub = n_->subscribe(camera_info_topic, 1, &PPFRecognizerDemo::camera_info_cb, this);
    ...
    bool callPPFRecognizerUsingFiles()
    {
        ...
        for(size_t i=0; i < test_cloud.size(); i++)
        {
            ...
            ppf_recognizer_ros_msgs::recognize srv_rec;
    ...
    {
        ros::init (argc, argv, "PPFRecognizerDemo");
        n_.reset( new ros::NodeHandle ( "~" ) );

        obj_rec_client_ = n_->serviceClient<ppf_recognizer_ros_msgs::recognize>("/object_recognition/recognize");
        obj_rec_set_camera_client_ = n_->serviceClient<ppf_recognizer_ros_msgs::set_camera>("/object_recognition/set_camera");
        ...
        if ( input_method_ == 0 )
        {
            ...
            if ( checkKinect() )
            {
                ...
                ros::Subscriber sub_pc = n_->subscribe (topic_, 1, &PPFRecognizerDemo::callPPFRecognizerUsingCam, this);
                ros::spin();
            }
            else
            {
