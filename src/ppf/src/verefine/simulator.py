...
class Simulator:
    ...
    def __init__(self, obj_ids, collider_paths, obj_coms, obj_scales):
        self.obj_ids = obj_ids
        self.collider_paths = collider_paths
        self.obj_coms = obj_coms
        self.obj_scales = obj_scales
        self.objects_to_use = obj_ids
    ...
    def _initialize_dataset(self):
        for i, (obj_id, model_path, obj_com, obj_scale) in enumerate(zip(self.obj_ids, self.collider_paths,
                                                                         self.obj_coms, self.obj_scales)):
            try:
                collision_shape = pybullet.createCollisionShape(shapeType=pybullet.GEOM_MESH,
                                                                fileName=model_path,
                                                                meshScale=[obj_scale]*3)
    ...
    def _world_to_bullet(self, in_world, id):
        ...
        offset = R @ np.array(self.obj_coms[self.obj_ids.index(id)])
    ...
    def _bullet_to_world(self, position, orientation, id):
        ...
        offset = in_world[0:3, 0:3] @ np.array(self.obj_coms[self.obj_ids.index(id)])
