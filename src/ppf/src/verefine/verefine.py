...
class Verefine:
    ...
    def __init__(self, obj_ids, meshes_sampled, refiner, simulator, renderer):
        self.obj_ids = obj_ids
        self.meshes_sampled = meshes_sampled
    ...
    def _refine_object(self, hypotheses, iterations=1, fixed=[], statistics=None):
        ...
        if mode > 1:
            model_id = self.obj_ids.index(hypotheses[0]['obj_id'])
        ...
        for _ in range(iterations):
            ...
            oi = self.obj_ids.index(str(hypothesis['obj_id']))
            T_ref = refiner.refine(T_cur, hypothesis['cloud_obs'], self.meshes_sampled[oi],
    ...
    def _refine_vfd(self, cluster_hypotheses):
        ...
        for iteration in range(iterations):
            ...
            score = renderer.compute_score(selected_ids, selected_poses,
                                           observation['extrinsics'], observation['intrinsics'], return_map=False)
            print("---")
    ...
    def _compute_dependencies(self, observation, frame_hypotheses):
        ...
        fat_masks = [binary_dilation(mask, iterations=5) for mask in masks]
        ...
        max_z = [cloud[:, 2].max() if cloud.shape[0] > 0 else None for cloud in clouds]
        min_d = [observation['depth'][np.logical_and(mask, observation['depth'] > 0)].min()
                 if np.logical_and(mask, observation['depth'] > 0).sum() > 0 else None for mask in masks]

        supports = np.zeros_like(adjacency, dtype=np.int8)
        occluded = np.zeros_like(adjacency, dtype=np.int8)
        for i in range(adjacency.shape[0]):
            if max_z[i] is None or min_d[i] is None:
                continue
            for j in range(adjacency.shape[0]):
                if max_z[j] is None or min_d[j] is None:
                    continue
